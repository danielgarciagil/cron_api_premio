...
docker build -t cron_api:$new_version .

# Etiquetar la imagen con la nueva versión
docker tag cron_api:$new_version carlosdiazz08/cron_api:$new_version
docker tag cron_api:$new_version carlosdiazz08/cron_api:latest

# Subir la imagen etiquetada al repositorio de Docker
if docker push carlosdiazz08/cron_api:latest && docker push carlosdiazz08/cron_api:$new_version; then
